...
<body>
    ...
    <? if(isset($modoEdicao) && $modoEdicao == true) { ?>
    <form action="?funcao=cadastra" method="post">

            <input type="hidden" name="id" id="id" value="<?= $contato->getId() ?>">

            <label for="nome">Nome:</label>
            <input name="nome" value="<?= $contato->getNome() ?>" id="nome" type="text" maxlength="70" placeholder="Preencha o nome do contato" required="required"/>

            <label for="email">E-mail:</label>
            <input name="email" value="<?= $contato->getEmail() ?>" id="email" type="email" placeholder="Preencha o email" required="required"/>

            <input type="submit" value="Atualizar" name="enviado"/>
